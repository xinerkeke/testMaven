...
public class testUserMan extends BaseTest{
	...
	String Institut[] = {"先进数通k", "2343","test","XX银行S", "2343",
			...
			+ "1111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111", "1234567","1234", 
			"~!@##@#$^%&*", "Test000", "Test000", "Tes t00 8", "Test10", "Test009", "Test12", 
			"Test12", "Test003","Test011", "XX银行一级支行1", "XX银行一级支行2", 
			"XX银行二级支行1", "XX银行二级支行2", "XX银行二级支行3", "XX银行二级支行4"};
	...
	public void testAdd(String loginName,String adminName,String email,
			String telephone,String mobilePhone,String expectStr) throws Exception {
		//增加测试代码,需按照界面要求修改方法参数
		 	
		    driver.findElement(By.xpath("//li/div/span[@class='tree-title' and text()='用户管理']")).click();//用户管理
		    
		    String addXpathBut =  "//div[@id='user-mainBody']/div[2]/div/div/div"
		    		+ "/div[@class='datagrid-toolbar']/a/span/span[text()='新增']";
		    
		    //显示等待"新增"按钮可以点击
		    timeDelayOpera(addXpathBut);
		    ...
		    assertTrue(isTextPresent(driver, expectStr));//断言执行
		    
	}
	
	
	@DataProvider(name = "testModUser")
	public static Object[][] getTestModData() throws Exception {
		return ReadExecl.initTestData("testuserMan.xls", "modUser");
	}
	 ...
	 * @throws Exception
	 */
	//@Test(dataProvider = "testModUser")
	public void testMod(String loginName,String adminName,String email,
			String telephone,String mobilePhone,String expectStr) throws Exception {
		driver.findElement(By.xpath("//li/div/span[@class='tree-title' and text()='用户管理']")).click();//用户管理
		
		Thread.sleep(5000);
		WebElement chkbox = divPage.getOperatedElement(driver, loginName, model);
		
		chkbox.findElement(By.name("userId")).click();
		
		String modPathBut =  "//div[@id='user-mainBody']/div[2]/div/div/div"
				+ "/div[@class='datagrid-toolbar']/a/span/span[text()='修改']";
		
		driver.findElement(By.xpath(modPathBut)).click();//修改
	    setUserInfo(loginName, adminName, email, telephone, mobilePhone);
		driver.findElement(By.xpath(".//*[@id='user-save']")).click();
	...
	@DataProvider(name = "testDelUser")
	public static Object[][] getTestDelData() throws Exception {
		return ReadExecl.initTestData("testuserMan.xls", "delUser");
	}
	 ...
	 * @throws Exception
	 */
	//@Test(dataProvider = "testDelUser")
	public void testDelUser(String loginName, String expectStr) throws Exception {

		driver.findElement(By.xpath("//ul[@id='topMenu']/li/a[text()='系统管理']")).click();
		driver.findElement(By.xpath("//li/div/span[@class='tree-title' and text()='用户管理']")).click();//用户管理
		WebElement chkbox = divPage.getOperatedElement(driver, loginName, model);
		
		chkbox.findElement(By.name("userId")).click();
		//取消删除
		chkbox.findElement(By.xpath("//div[@class='datagrid-toolbar']/a[3]/span/span[text()='删除']")).click();
		chkbox.findElement(By.xpath("//input[@class='imoia_dap_btn_closed' and @value='取消']")).click();
		
		Thread.sleep(500);
		
		//确定删除
		chkbox.findElement(By.xpath("//div[@class='datagrid-toolbar']/a[3]/span/span[text()='删除']")).click();
		chkbox.findElement(By.xpath("//input[@id='popConfirmYes' and @value='确认']")).click();
	...
	private void setUserInfo(String loginName, String adminName, String email, String telephone, String mobilePhone) {
		...
		//选择用户类型
		driver.findElement(By.xpath("//form[@id='userEditForm']/div[@class='imoia_pop_con']"
				+ "/ul/li[4]/span/span/span/span")).click();
		String userTypeXpath = "//body[@id='mainBody']/div[15]/div/div";
		getOperateElement(userTypeXpath).click();
		 
/*		driver.findElement(By.xpath("//form[@id='userEditForm']/div[@class='imoia_pop_con']/ul/"
				+ "li[5]/span/span/span")).click();
		//driver.findElement(By.xpath("//body[@id='mainBody']/div[16]/div/ul/li[2]/div/span[3]")).click();
		String instituXpath = "//body[@id='mainBody']/div[16]/div/ul/li";
		getOperateElement(instituXpath, "ul/li").findElement(By.xpath("./div/span[3]")).click();
*/		
		//设置"所属机构"
		setInstituteVal();
	...
	private void setInstituteVal() {
		//获取所属机构元素
		WebElement ele = driver.findElement(By.xpath("//form[@id='userEditForm']/div[@class='imoia_pop_con']/ul/"
				+ "li[5]/span/input"));
		//删除"readonly"只读属性
   ...
   //随机选择操作元素
	...
	private WebElement getOperateElement(String xpath,String chilsListXpath) {
		...
		//获取xpath路径下的元素list
		List<WebElement>list = driver.findElements(By.xpath(xpath));
		int size = list.size();   //列表元素个数
		
		//随机获取tmpRand+1个元素
		int tmpRand = rand.nextInt(size);  
		WebElement element = list.get(tmpRand);
		
		for(;;){
			//如果随机数==元素列表个数，返回当前元素，对进行操作
			if((rand.nextInt(size+1))== size)
			{
			    break;
			}
			//点击"下级"列表”
			...
			if(childList.size() == 0)
			{
				element = list.get(tmpRand);
				break;
			}
			
			size = childList.size();
			tmpRand = rand.nextInt(size);
			element = childList.get(tmpRand);
		}
		return element;
	}
	*/
